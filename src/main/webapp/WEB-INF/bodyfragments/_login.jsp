...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize text-indigo-800 active" aria-current="page"> <i class="fa fa-arrow-right text-indigo-800" aria-hidden="true"> Login</i></li>
  </ol>
</nav>
	  ...
	  <div class="md:flex w-full">
		...
		<div class="bg-white  w-full md:w-1/2 py-10 px-5 md:px-10">
		  <div class="text-center mb-10">
			<h1 class="font-bold text-2xl text-gray-900">Sign in to your account</h1><br></br>
			<b><%@ include file="businessMessage.jsp"%></b>
				<sf:form role="form" action="${pageContext.request.contextPath}/login" method="post" modelAttribute="form">
				
				<sf:hidden path="${sessionScope.user.firstName}" id="name-value"/>
					...
					<s:bind path="userName">
							...
							<s:bind path="password">
								<div class="relative mb-6">
									<div class="relative">
								<label for="exampleInputPassword1" class="absolute text-sm text-black dark:text-black duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-gray-100 dark:bg-gray-300 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1">PASSWORD<font color="red">*</font></label>
								<sf:input type="password" path="${status.expression}"
									class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" />
									<div class="form-text"><font color="red" style="font-size: 13px"><sf:errors path="${status.expression}"/></font></div>
								</div></div>
							</s:bind>
							<h6><input type="submit" name="operation" class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold"
								value="SIGN IN"><br></br>
							<a href="${pageContext.request.contextPath}/forgetPassword" class="cursor-pointer text-blue-600">&nbsp;&nbsp;Forget Password ?</a></h6>
				</sf:form>
		</div>
	</div>
</div>
